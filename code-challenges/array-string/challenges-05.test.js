...
const howMuchPencil = (str) => {
  ...
  // This cannot be inside for loop str.length + 1 because str length is changing everytime it got sliced.
...
const listFoods = (recipe) => {
  ...
  gruffaloCrumble.ingredients.forEach(ingredient => {
    ...
    // Replace quantity number with '' to get rid of it
    ingredient = ingredient.slice(ingredient.indexOf(' ') + 1);
    // Grab the first location of space ' ' then slice it. Since we don't need the space, slice 1 more to get rid of space.
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    ...
    // For every ingredient split them into number, unit, and name using split(). Then slice the first two elements which are number and unit. Then join them back together. Use repl.it to see.
    // p.s. split('') vs split(' '): first will only split them apart, second will split and the space is gone.
  })
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  let originalStr = str;
  let len = originalStr.length;
  if (len <= numberOfCharacters) {
    let str = originalStr.slice(len);
    return str;
  }
    ...
    return originalStr.slice(0, -numberOfCharacters);
  }
};

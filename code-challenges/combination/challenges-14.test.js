...
let biggerThanLuke = (arr) => {
  ...
  const person = arr.find(obj => {
    if (obj.name === 'Luke Skywalker') {
      return obj;
    }
  });

  const targetMass = person.mass;
  const greaterMass = arr.filter(obj => {
    if (Number(obj.mass) > Number(targetMass)) {
      return obj.name;
    }
  }).map(obj => {
    return obj.name;

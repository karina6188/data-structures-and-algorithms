...
const hasChildrenEntries = (arr, character) => {
  for (let i = 0; i < arr.length; i++) {
    if (character === arr[i].name) {
      let check = Object.entries(arr)[i][1].children;
      if (check.length > 0) {
        return true;
      }
      else {
...
const totalCharacters = (arr) => {
  let allCharacters = [];
  for (let i = 0; i < arr.length; i++) {
    let character = Object.entries(arr)[i][1].name;
    let spouse = Object.entries(arr)[i][1].spouse;
    let children = Object.entries(arr)[i][1].children;
    allCharacters.push(character);
    if (spouse) {
      allCharacters.push(spouse);
    }
    if (children) {
      children.forEach(child => {
        allCharacters.push(child);
      });
    }
  }
